...
impl BDPTStep {
    fn get_rt_bgl(device: &wgpu::Device) -> wgpu::BindGroupLayout {
        let bgl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("bgl for metropolis.wgsl"),
            entries: &[
    ...
    pub fn create_static_bind_groups(&mut self, state: &wgpu_util::WGPUState, scene: &scene::Scene) {
        ...
        let rt_bgl = BDPTStep::get_rt_bgl(&state.rt_device);
        ...
        let texture_bgl = BDPTStep::get_texture_bgl(&state.rt_device, &scene);

        let mut views = Vec::new();
        let mut samplers = Vec::new();
        for tex in &scene.textures {
            let texture = tex.lock().unwrap();
            views.push(texture.view.as_ref().unwrap().clone());
            samplers.push(texture.sampler.as_ref().unwrap().clone());
        }
            ...
            entries: &[
                wgpu::BindGroupEntry {
                    ...
                    resource: wgpu::BindingResource::TextureViewArray(
                        views.iter().map(|v| v).collect::<Vec<&wgpu::TextureView>>().as_slice()
                    ),
                },
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::SamplerArray(
                        samplers.iter().map(|s| s).collect::<Vec<&wgpu::Sampler>>().as_slice()
                    ),
                },
            ],
        });
...
impl RenderStep for BDPTStep {
    fn create(wgpu_state: &mut wgpu_util::WGPUState, scene: &scene::Scene) -> Self {
        let rt_bgl = BDPTStep::get_rt_bgl(&wgpu_state.rt_device);
        let texture_bgl = BDPTStep::get_texture_bgl(&wgpu_state.rt_device, &scene);
        let uniform_bgl = BDPTStep::get_uniform_bgl(&wgpu_state.rt_device);
        ...
        let shader = shader::create_shader_compute(wgpu_state, &wgpu_state.rt_device, "shaders/pathtracer_bidirectional/pathtracer_bidirectional.wgsl");
